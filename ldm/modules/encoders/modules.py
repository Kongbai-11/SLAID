...
class FrozenOpenCLIPImageEncoder(AbstractEncoder):
    ...
    def set_trainable_components(self, train_projector=False):
        projector_modules = [self.projector_token, self.projector_embed]
...
class FrozenDinoV2Encoder(AbstractEncoder):
    ...
    def encode_window_refs(self, ref_windows, ref_weights=None, window_meta=None, window_mask=None):
        squeeze_batch = False
        if isinstance(ref_windows, list):
            ref_windows = torch.stack(ref_windows, dim=0)

        if ref_windows.dim() == 4:
            ref_windows = ref_windows.unsqueeze(0)
            squeeze_batch = True

        if ref_windows.shape[-1] == 3:
            ref_windows = ref_windows.permute(0, 1, 4, 2, 3).contiguous()

        batch_size, num_windows = ref_windows.shape[:2]
        ref_windows = ref_windows.to(self.device, dtype=torch.float32)
        flat_windows = ref_windows.view(batch_size * num_windows, *ref_windows.shape[2:])
        cond_tokens = self._encode_tokens(flat_windows)
        cond_tokens = cond_tokens.view(batch_size, num_windows, cond_tokens.shape[1], cond_tokens.shape[2])

        if ref_weights is None:
            ref_weights = cond_tokens.new_ones(batch_size, num_windows)
        elif ref_weights.dim() == 1:
            ref_weights = ref_weights.unsqueeze(0)

        if window_mask is not None and window_mask.dim() == 1:
            window_mask = window_mask.unsqueeze(0)

        if window_meta is not None and window_meta.dim() == 2:
            window_meta = window_meta.unsqueeze(0)

        ref_weights = ref_weights.to(cond_tokens.device, dtype=cond_tokens.dtype)
        if window_mask is not None:
            ref_weights = ref_weights * window_mask.to(cond_tokens.device, dtype=cond_tokens.dtype)
        ref_weights = ref_weights / ref_weights.sum(dim=1, keepdim=True).clamp(min=1e-6)
        fused = (cond_tokens * ref_weights[:, :, None, None]).sum(dim=1)
    ...
    def forward(self, image):
        if isinstance(image,list):
            image = torch.cat(image,0)
        return self._encode_tokens(image)
